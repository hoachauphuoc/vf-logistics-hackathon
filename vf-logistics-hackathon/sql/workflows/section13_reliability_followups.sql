...
-- ---------------------------------------------------------------------------
-- 13d - residual, deliberately not fixed
--
-- A 7-day window can itself fill, and the gate still suppresses ALL severities
-- when it trips, so a saturated queue of MEDIUM alerts can still delay
-- HIGH-severity detection. Letting the HIGH rules bypass the gate means
-- restructuring the IF/ELSE around five separate INSERT statements, and an
-- unverified structural rewrite is not worth making this close to the deadline.
-- The window fix removes the permanent latch, which was the actual defect.
-- ---------------------------------------------------------------------------
