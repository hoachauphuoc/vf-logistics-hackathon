...
`TASK_COMPLIANCE_CHECK` needed two further corrections. `CHECK_COMPLIANCE` writes to `BILL_OF_LADING`, the very table its stream watches, so the task re-armed its own condition and would have re-checked the same row forever; the stream is now filtered to `METADATA$ACTION = 'INSERT'`. More seriously, its original body picked *any* row with `COMPLIANCE_CHECK_PASSED IS NULL` — and **all 10,017 rows were unchecked**, so at one row per 30 minutes it was a **209-day backfill driven by its own writes rather than by new data**, on an account that expires 2026-09-04. Draining history is a bulk operation, not an event-driven task, so new arrivals are now copied into a small `COMPLIANCE_QUEUE` as the stream is consumed (without which any arrival beyond a single run's limit would be silently discarded), and the historical backlog was drained separately in one set-based pass — see *Fix 1c*, which also explains why running the old procedures would not have drained anything at all.
...
### Fix 1c: Compliance checks that never checked anything
Asking what it would cost in credits to compliance-check the 10,017 historical B/Ls surfaced something worse than a cost. The cost answer, measured rather than estimated: **`CHECK_COMPLIANCE` contains no Cortex call at all**, so Cortex spend is zero; row-by-row it is 1,300 ms per B/L (measured over 8 real calls) ≈ 3.6 hours ≈ **3.6 credits** on an X-Small warehouse, and the same three rules expressed set-based took **314 ms for all 10,017 rows ≈ 0.000087 credits** — roughly 41,000× cheaper.

But spending either amount would have bought nothing:

- **Nothing in the schema ever set `BILL_OF_LADING.COMPLIANCE_CHECK_PASSED`.** `CHECK_COMPLIANCE` only inserted into `COMPLIANCE_CHECK_RESULT`, so the "unchecked" count was unaffected by running it and the scheduled task would re-select the same B/L forever. Already visible before the fix: 8 result rows covering only 4 distinct B/Ls.
- **`BATCH_CHECK_COMPLIANCE` evaluated no rules whatsoever.** It aggregated the existing flag and counted `NULL` as passed, returning `{"failed":0,"passed":10017,"total":10017}` — a clean bill of health for 10,017 shipments that had never been examined. Running the rules for real shows **1,351 of them (13.5%) fail**. For a compliance submission that is the most serious class of defect available: a false assurance, not a missing feature.
- Its `P_BATCH_SIZE` was inert (a `LIMIT` on an aggregate query returning one row), and with the page's 24-hour window no row qualified, so the Bulk Scan button returned `{}` and the UI displayed "0 passed, 0 failed".
- The procedure returned `compliant` and `violations` while the page reads `status` and `issues`, so **every single-B/L check displayed as FAILED regardless of the real result**, and the violation list never rendered.
- `COMPLIANCE_CHECK_RESULT.CHECK_ID` carried the same unenforced-primary-key trap as `FRAUD_ALERT` (`autoincrement start 300` against seeded ids reaching 402); one duplicate already existed, and inserting 10,017 rows would have added about a hundred more.
...
Applied SQL, with assertions: `sql/workflows/section13_reliability_followups.sql` and `sql/workflows/section13d_severity_aware_throttle.sql`.

**The gate no longer blinds `HIGH` detection either.** Windowing the queue removed the permanent latch but left the gate wrapping all five detection rules, so whenever it *did* trip it suppressed every severity — a saturated backlog of `MEDIUM` alerts would still hide a `HIGH`-severity fraud. Backpressure is meant to defer low-value work, not to stop looking for the serious cases. The gate is now advisory: `SUSPICIOUS_PARTY` (unconditionally `HIGH`) always runs; the three rules that pick severity with a `CASE` are narrowed under saturation to exactly the rows that `CASE` would have marked `HIGH`, with each guard copied from the `CASE` in its own `INSERT` so the two cannot drift apart; `WEIGHT_ANOMALY` (unconditionally `MEDIUM`) is suppressed whole. The return value gained an additive `medium_suppressed` key, because `throttled: true` no longer implies nothing was detected.

The obvious test for this proves nothing, which is worth recording. Forcing saturation with `WORKFLOW_DETECT_AND_ACT(1)` returned `new_alerts: 0` — but counting HIGH-eligible candidates showed **0 for all four HIGH-capable rules**, so zero was the right answer for a reason unrelated to the fix, and is exactly what a broken guard would also produce. It was therefore tested against injected data: one B/L that can only raise `HIGH` (suspicious shipper) and one that can only raise `MEDIUM` (twice p99 weight, ordinary charges). Saturated, the `HIGH` row was detected and the `MEDIUM` row was not; unsaturated, the `MEDIUM` row was picked up as `WEIGHT_ANOMALY`. Before the change **both** would have been missed. All test rows were then deleted and the 496-row baseline reconfirmed.
...
| Compliance backfill not started by stealth | `COMPLIANCE_QUEUE` holds only genuinely new arrivals while the historical backlog is handled explicitly by Bulk Scan |
| Compliance flag is now persisted | `COMPLIANCE_CHECK_PASSED` set on all 10,017 rows: 8,666 pass / 1,351 fail / 0 unchecked. Previously 0 rows had it set no matter how often the check ran |
| Compliance flag agrees with the rules | 0 rows where the stored flag differs from a fresh evaluation of the three rules |
| Bulk Scan no longer reports a false pass | Returns `{"passed":0,"failed":0,"total":0,"not_checked_remaining":0}` once everything is checked; previously returned `passed: 10017` for shipments never examined |
...
| A full queue cannot hide a HIGH-severity fraud | With the gate forced shut (`P_QUEUE_LIMIT = 1`), an injected suspicious-shipper B/L was still detected as `HIGH / SUSPICIOUS_PARTY` while an injected MEDIUM-only B/L was correctly suppressed; unsaturated, the MEDIUM one was then picked up as `WEIGHT_ANOMALY`. Before the fix both were missed |
| Severity guards match their own `CASE` | 4 guards present, one per severity-choosing rule, each copied from the `CASE` in the same `INSERT` |
...
| Leaked evaluator password is dead | Rotated and verified by login attempt: the value published in git history is rejected (`errno 250001`), the replacement authenticates as `HACKATHON_JUDGE` / `HACKATHON_JUDGE_ROLE` |
| No sign the leak was exploited | `LOGIN_HISTORY_BY_USER` over the exposure window shows one password authentication, from the developer's own IP; every other entry is an internal Streamlit session token (`0.0.0.0`, `OAUTH_ACCESS_TOKEN`) |
| Private key never committed | 0 tracked files under `keys/`; the directory itself is now gitignored rather than relying on the `*.p8` pattern |
